...
test('resolveCScripts resolves to src/index.js when in the c-scripts package', () => {
  mockPkg({package: {name: 'c-scripts'}})
  expect(require('../utils').resolveCScripts()).toBe(
    require.resolve('../').replace(process.cwd(), '.'),
  )
})

test('resolveCScripts resolves to c-scripts if not in the c-scripts package', () => {
  mockPkg({package: {name: 'not-c-scripts'}})
  whichSyncMock.mockImplementationOnce(() => require.resolve('../'))
  expect(require('../utils').resolveCScripts()).toBe('c-scripts')
})
